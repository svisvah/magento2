...
class ToolbarPlugin
{
    ...
    public function afterSetCollection(Subject $subject, $result, ProductCollection $collection)
    {
        ...
        if ($customerGroupId == 2) {
            $collection->addAttributeToFilter('wholesale_visibility', ['eq' => 1]); // Show products where wholesale_visibility is "no"
        }
        else
        {
            $collection->addAttributeToFilter('wholesale_visibility', ['eq' => 0]); 
